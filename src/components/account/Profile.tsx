...
export default function Profile() {
  ...
  function handleUpdateAdminCode() {
    ...
    try {
      axios
        .put("/api/account/root", { companyId: company?.id, role: "ADMIN" })
        .then((res) => {
          console.log(res);
  ...
  function handleUpdateUserCode() {
    ...
    try {
      axios
        .put("/api/account/root", { companyId: company?.id, role: "USER" })
        .then((res) => {
          console.log(res);
  ...
  function inviteUser(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      setLoading(true);
      axios
        .post(`${process.env.NEXT_PUBLIC_EMAIL_URL}/api/Email`, {
          to: inviteEmail,
  ...
  return (
    <div>
      {showSucess && (
        <GreenAlert
          AletTitle="Success: "
          alertText=" User invitation sent"
          showAlert={showSucess}
          setShowAlert={setShowSucess}
        />
      )}
      {showError && (
        <RedAlert
          AletTitle="Error: "
          alertText=" Please enter both email and role to invite user"
          showAlert={showError}
                        ...
                        <div className="w-full flex md:flex-row flex-col px-4 py-2 space-y-2 justify-around">
                          <div>
                            <input
                              ...
                              id="emailAddress"
                              type="email"
                              className="mt-2 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-[#9FDF97] focus:border-[#9FDF97] block w-full p-2.5"
                            />
                          </div>
                          ...
                          <div>
                            ...
                            >
                              ...
                              <option value={company.admin_code?.toString()}>
                                ADMIN
                              </option>
                              <option value={company.user_code?.toString()}>
                                USER
                              </option>
                            </select>
                          </div>
                        ...
                        <div className="mt-5 w-full flex flex-col items-center overflow-hidden text-sm">
                          <div className="flex justify-between w-full border-t border-gray-100 text-gray-600 py-4 pl-6 pr-3 w-full block hover:bg-gray-100 transition duration-150">
                            ...
                            {user.role === "ROOT" && (
                              <CiEdit
                                ...
                                className="text-2xl hover:text-[#80CF76]"
                              />
                            )}
                          </div>
                          ...
                          <div className="flex justify-between w-full border-t border-gray`-100 text-gray-600 py-4 pl-6 pr-3 w-full block hover:bg-gray-100 transition duration-150">
                            <div>
                              ...
                              <p className="font-bold">
                                {(company.city as Prisma.CitySelect)?.city ??
                                  ""}
                              </p>
                            ...
                            <>
                              <div className="flex justify-between w-full border-t border-gray-100 text-gray-600 py-4 pl-6 pr-3 w-full block hover:bg-gray-100 transition duration-150">
                                ...
                                <RxUpdate
                                  ...
                                  className="text-xl hover:text-[#80CF76]"
                                />
                              </div>
